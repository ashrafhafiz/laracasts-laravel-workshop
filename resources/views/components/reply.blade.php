        ...
        @if ($showEngagement)
            <div class="mt-6 flex items-center justify-between gap-4">
                <div class="flex items-center gap-8">
                    <!-- Like -->
                    <x-like-button :id="$post->id" :active="$post->has_liked" :count="$post->likes_count ?? 0" />

                    <!-- Comment/reply -->
                    <x-reply-button :id="$post->id" :count="$post->replies_count ?? 0" />

                    <!-- Re-post -->
                    <x-repost-button :id="$post->id" :active="$post->has_reposted" :count="$post->reposts_count ?? 0" />
