        ...
        <div class="border-pixl-light/10 border-b pb-5">
            ...
            <div class="[&_a]:text-pixl mt-4 flex flex-col gap-3 text-sm [&_a]:hover:underline">
                ...
                @if ($post->isRepost() && $post->content != null)
                    <ul>
                        <x-post :post="$post->repostOf" :showEngagement="false" />
                    </ul>
                @endif
            ...
            @if ($showEngagement)
                <div class="mt-6 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-8">
                        <!-- Like -->
                        <x-like-button :id="$post->id" :active="$post->has_liked" :count="$post->likes_count" />

                        <!-- Comment/reply -->
                        <x-reply-button :id="$post->id" :count="$post->replies_count" />

                        <!-- Re-post -->
                        <x-repost-button :id="$post->id" :active="$post->has_reposted" :count="$post->reposts_count" />
                    </div>
